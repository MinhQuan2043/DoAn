...
public class HotelSearchController {
    ...
    public String findAvailableHotelsByCityAndDate(@Valid @ModelAttribute("hotelSearchDTO") HotelSearchDTO hotelSearchDTO, BindingResult result) {
        ...
        return String.format("redirect:/search-results?city=%s&checkinDate=%s&checkoutDate=%s", URLEncoder.encode(hotelSearchDTO.getCity(), StandardCharsets.UTF_8), hotelSearchDTO.getCheckinDate(), hotelSearchDTO.getCheckoutDate());
    }
    ...
    public String showSearchResults(@RequestParam String city, @RequestParam String checkinDate, @RequestParam String checkoutDate, Model model, RedirectAttributes redirectAttributes) {
        try {
            ...
            String cityDecode= URLDecoder.decode(city, StandardCharsets.UTF_8);
            ...
            log.info("Searching hotels for city {} between dates {} and {}", cityDecode, checkinDate, checkoutDate);
            List<HotelAvailabilityDTO> hotels = hotelSearchService.findAvailableHotelsByCityAndDate(cityDecode, parsedCheckinDate, parsedCheckoutDate);
            ...
            model.addAttribute("city", cityDecode);
